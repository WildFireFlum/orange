...
template<typename K>
PQ<K>::PQ() : m_thread_count(0) {}


template<typename K>
bool PQ<K>::checkRebalance(Chunk<K> &C, K& key) {
...
template<typename K>
Chunk &PQ<K>::locate_target_chunk(const K& key) {
...
template<typename K>
bool PQ<K>::try_pop(K &key) {
    return false;
}

template<typename K>
bool PQ<K>::push(const K &key) {
