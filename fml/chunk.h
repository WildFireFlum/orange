...
enum checkRebalncedResult {
    NOT_REQUIRED, ADDED_KEY_VAL, NOT_ADDED_KEY_VAL
};
...
struct K_Element {
    ...
    K_Element(uint64_t key, K_Element* next, V* value) : m_key(key), m_next(next), m_value(value) {}
...
Chunk::Chunk(uint64_t min_key, Chunk* rebalance_parent) : m_min_key(min_key), m_next(nullptr),
                                 m_end_sentinel(UINT_FAST64_MAX, nullptr, nullptr), m_begin_sentinel(0, &m_end_sentinel, nullptr),
                                 m_rebalance_status(INFANT), m_rebalance_parent(rebalance_parent), m_rebalance_data(&nullRebalancedData) {
...
bool Chunk<V>::rebalance(uint64_t key, V &val) {
    ...
    if (!this->m_rebalance_data.compare_exchange_strong(nullRebalancedDataPtr, ro)) {
        free(ro);
    }
    ...
    while (chunk) {
        ...
        for (int i = 0; i < CHUNK_SIZE; i ++) {
            ...
            uint64_t z;
            while ( !((z = k->m_key) & FREEZE_MASK) && !k->m_key.compare_exchange_strong(z, z | FREEZE_MASK));
        }
    }
    ...
    do {
        for (int i = 0; i < CHUNK_SIZE; i++) { // TODO: probably i < min(CHUNK_SIZE, this->m_count) is better than that
            if (!(chunk->m_k[i].m_key & DELETED_MASK)) {
                v.push_back(chunk->m_k[i]); // TODO: maybe we don't want to copy them at this point
            }
        }
    ...
    for (auto &k : v) {
        ...
        *Cn->m_k[Cn->m_count].m_value = *k.m_value;              // copy the value
        Cn->m_k[Cn->m_count].m_next = &Cn->m_k[Cn->m_count + 1]; // set list pointer
        Cn->m_count++;
    }
    ...
    } while (! last->m_next.compare_exchange_strong(Cn->m_next + false, Cn->m_next + true));

    Chunk* pred = this predecessor; // TODO: read in the paper how they get the predecessor...
...
bool Chunk<V>::put(uint32_t key, V& val) {
    ...
    this->m_v[i] = val; // TODO: CAS is not needed but we need something like volatile in java here
    uint64_t z = this->m_k[i].m_key;

    if ((z & FREEZE_MASK) || !this->m_k[i].m_key.compare_exchange_strong(z, unique_key)) {
        return this->rebalance(unique_key, val);
    }

    // we successfully added the key to m_k - TODO: connect it to the sorted list of the chunk
...
bool Chunk<V>::delete_min(std::pair<uint32_t, V> *out) {
    ...
    while (curr < &this->m_end_sentinel) {
        if (((z = curr->m_key) & FLAGS_MASK) && curr->m_key.compare_exchange_strong(z, z | DELETED_MASK)) {
